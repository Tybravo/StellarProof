...
The Verification Pipeline is the core logic of the StellarProof smart contract, responsible for processing TEE (Trusted Execution Environment) attestations and updating the state of verification requests.
...
| `InvalidSignature` | The cryptographic signature does not match the attestation payload. | `Rejected` (Host Abort) |
